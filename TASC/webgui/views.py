            ...
            if True == True:
                if ratelimit > 0:
                    ...
                    if "ajax" in request.POST.keys() and request.POST["ajax"] == "True":
                        ...
                        history["notifications"].insert(0, "{} started at {}".format(request_type,
                                                                                     datetime.now().astimezone(
                                                                                         tz.gettz('ITC')).strftime(
                                                                                         '%H:%M %d %b')))
                        with open("media/json/history_{}.json".format(username), "w") as file:
                            file.write(json.dumps(history, indent=4))
...
def social(request, request_type, request_data, googlemapapikey):
    ...
    if request_type == 'facebook':
        ...
        if "ajax" in request.POST.keys() and request.POST["ajax"] == "True":
            history["Search_query"].insert(0, {"query": ":".join(query),
                                               "time": datetime.now().astimezone(tz.gettz('ITC')).strftime(
                                                   '%H:%M %d %b')})
            history["notifications"].insert(0, "{} ended at {}".format(request_type, datetime.now().astimezone(
                tz.gettz('ITC')).strftime('%H:%M %d %b')))
            with open("media/json/history_{}.json".format(username), "w") as file:
                file.write(json.dumps(history, indent=4))
            return JsonResponse(history["notifications"], safe=False)
        ...
        if request_data in data[request_type].keys():
            ...
            data[request_type][request_data] = {'fbdata': fbdata, 'instadata': instadata, 'twitterdata': twitterdata,
                                                'gitdata': gitdata, "tinder": tinderdata, "whatname": whatname,
        ...
        return render(request, 'social.html',
                      {'fbdata': fbdata, 'instadata': instadata, 'twitterdata': twitterdata, 'gitdata': gitdata,
                       "tinder": tinderdata, "whatname": whatname, 'gravatar': gravatardata,
                ...
                if len(filename) == 2:
                    if filename[-1] in ['jpg', 'png', 'gif', 'tif', 'jpeg']:
                        ...
                        if 'metaimage' in request.FILES.keys() and str(request.FILES['metaimage']) in metafiles.keys():
                            ...
                            user.profile.save()
                            history["app_type"]["metadata"].insert(0, {"filename": str(request.FILES['metaimage']),
                                                                       "time": datetime.now().astimezone(
                                                                           tz.gettz('ITC')).strftime('%H:%M %d %b')})
                            with open("media/json/history_{}.json".format(username), "w") as file:
                                file.write(json.dumps(history, indent=4))
                        ...
                        if 'Error' in metadata.keys():
                            return render(request, 'apps/metadata.html', {'metadata': metadata, "POST": "post",
                                                                          "history": history["app_type"]["metadata"][
                                                                                     :10]})
                            ...
                            return render(request, 'apps/metadata.html',
                                          {'metadata': metadata, 'gmap3': gmap3, "POST": "post",
                                           "history": history["app_type"]["metadata"][:10]})
                        else:
                            return render(request, 'apps/metadata.html', {'metadata': metadata, "POST": "post",
                                                                          "history": history["app_type"]["metadata"][
                                                                                     :10]})
                        ...
                        return render(request, 'apps/metadata.html',
                                      {"Error": "Upload a filename with Valid Extension"})
                else:
